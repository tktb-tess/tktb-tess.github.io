...
となる。よって $a_3 x^3 + a_2 x^2 + a_1 x + a_0 = 0$ の解は
...
つまり $u^3, v^3$ が虚数であることと、$D > 0$ すなわち3次方程式が3つの実数解をもつことは、実は同値なのである。さらに3次方程式が既約な場合、虚数を避けて代数的に解を書き下すことはできないことが分かっており、これを **還元不能** という。解が全て実数解なのに虚数の立方根の和でしか書けないというのはなかなか不思議であると私は思う。
...
まずTartaglia–Cardano–del Ferroの解法で解いてみる。
...
Vièteの解法でも求めてみる。$t = A\cos{\theta}$ とおいて代入。

$$
\begin{alignedat}{2}
&(A\cos{\theta})^3 - 15A\cos{\theta} + 6 = 0 \\ \\
&\cos^3{\theta} - \frac{15}{A^2} \cos{\theta} + \frac{6}{A^3} = 0
\end{alignedat}
$$
...
ここで、左辺が (2乗) - (2乗) の形になれば、左辺は2つの2次式に因数分解出来て2次方程式に帰着できる。よって第2項 $(2\mu - c_2)t^2 - c_1 t + \mu^2 - c_0$ が完全平方になるように $\mu$ の値を決める。これは2次方程式 (第2項) = 0 が重解をもつこと、またその判別式が0であることと同値。判別式 $D$ は

$$
\begin{alignedat}{2}
D &= (-c_1)^2 - 4(2\mu - c_2)(\mu^2 - c_0) \\
...
$$
\begin{alignedat}{2}
&(t - 3)^4 + 12(t - 3)^3 + 4(t - 3)^2 - 16(t - 3) - 8 = 0 \\
&(t^4 - 12t^3 + 54t^2 - 108t + 81) + 12(t^3 - 9t^2 + 27t - 27) + 4(t^2 - 6t + 9) - 16(t - 3) - 8 = 0 \\
&t^4 - 12t^3 + 54t^2 - 108t + 81 + 12t^3 - 108t^2 + 324t - 324 + 4t^2 - 24t + 36 - 16t + 48 - 8 = 0 \\
&t^4 - 50t^2 + 176t - 167 = 0
\end{alignedat}
$$
...
$$
\begin{alignedat}{2}
&t^4 + \mu^2 + 2\mu t^2 - \mu^2 - 2\mu t^2 - 50t^2 + 176t - 167 = 0 \\
&(t^2 + \mu)^2 - (2\mu + 50)t^2 + 176t - \mu^2 - 167 = 0 \\
&(t^2 + \mu)^2 - \{(2\mu + 50)t^2 - 176t + \mu^2 + 167\} = 0
\end{alignedat}
$$

これの第2項が完全平方になればよい。$t$ についての2次方程式 $(50 - 2\mu)t^2 - 176t + \mu^2 + 167 = 0$ の判別式を $D$ とおくと

$$
\begin{alignedat}{2}
D &= (-88)^2 - (2\mu + 50)(\mu^2 + 167) \\
&= 7744 - 2\mu^3 - 334\mu - 50\mu^2 - 8350 \\
&= - 2\mu^3 - 50\mu^2 - 334\mu - 606 \\
&= -2(\mu^3 + 25\mu^2 + 167\mu + 303)
\end{alignedat}
$$

よって $\mu$ は3次方程式 $\mu^3 + 25\mu^2 + 167\mu + 303 = 0$ を満たす。

解を一つ（がんばって）探すと、$\mu = -3$ が解になる事が分かる。よって、元の4次方程式は

$$
\begin{alignedat}{2}
&(t^2 + \mu)^2 - \{(2\mu + 50)t^2 - 176t + \mu^2 + 167\} = 0 \\
&(t^2  - 3)^2 - (44t^2 - 176t + 176) = 0 \\
&(t^2  - 3)^2 - 44(t - 2)^2 = 0 \\
&\{(t^2  - 3) - 2\sqrt{11}(t - 2)\}\{(t^2  - 3) + 2\sqrt{11}(t - 2)\} = 0 \\
&(t^2 - 2\sqrt{11}t + 4\sqrt{11} - 3)(t^2 + 2\sqrt{11}t - 4\sqrt{11} - 3) = 0
\end{alignedat}
$$

$t^2 - 2\sqrt{11}t + 4\sqrt{11} - 3 = 0$ の解は

$$
t = \sqrt{11} \pm \sqrt{14 - 4\sqrt{11}}
$$

同様に $t^2 + 2\sqrt{11}t - 4\sqrt{11} - 3 = 0$ の解は

$$
t = -\sqrt{11} \pm \sqrt{14 + 4\sqrt{11}}
$$

よって $x^4 + 12x^3 + 4x^2 - 16x - 8 = 0$ の解は

$$
\begin{alignedat}{2}
x = -&3 + \sqrt{11} \pm \sqrt{14 - 4\sqrt{11}}, \\
-&3 - \sqrt{11} \pm \sqrt{14 + 4\sqrt{11}}
\end{alignedat}
$$
